...
class Main(QMainWindow):
    ...
    def front_inter(self, num): 
        ...
        if num == 1 and self.int1_set == True:
            pass
    ...
    def confirm(self):
        ...
        safeDis = self.inter.spinBoxSafDisFro.value()
        ...
        self.intersection_list[self.cur_inter].edit_vehicle_settings(self.cur_ID[0], command=turn, stop_choice=stop, vehicle_color=color, gap=gap, safety_distance=safeDis, obey_traffic_lights=state, choice=choice_string, penetrate_distance=self.penetrate_dis)
    ...
    def change_inter(self):
        ...
        saved_file = self.intersection_list[self.cur_inter].export_settings()
        self.saved_intersections[self.cur_inter] = saved_file
        #self.inter.cobBoxImportInt.setCurrentIndex(self.inter.cobBoxIDInt.currentIndex())
